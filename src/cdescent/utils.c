...
cdescent_copy_beta (const cdescent *cd, bool scaling)
{
	...
	if (scaling && !cdescent_is_regtype_lasso (cd)) {
		...
		if (cdescent_is_regtype_ridge (cd)) {
			double	scale2 = 1. + lambda2;
			dscal_ (LINREG_CINTP (p), &scale2, beta, &ione);
		} else {
			int		j;
			for (j = 0; j < p; j++) beta[j] *= (1. + cd->jtj[j] * lambda2);
		}
	}
	return beta;
}
