...
mm_mtx		*mm_mtx_copy (const mm_mtx *mm);

void		mm_array_set_all (int n, double *data, const double val);
void		mm_mtx_set_all (mm_mtx *mm, const double val);

mm_dense	*mm_mtx_sparse_to_dense (const mm_sparse *s);
mm_sparse	*mm_mtx_dense_to_sparse (const mm_dense *d, const double threshold);
